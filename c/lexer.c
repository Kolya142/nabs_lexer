#include "lexer.h"
#
Token *token_new(str value, i32 type, u32 line) {
...
Token* token_append(Token *token, str value, i32 type, u32 line) {
...
Token *lexer(str code) {
    ...
    U8 context = 0 ;
    Token* first = token_new(NULL, TokenType_START, 0);
    Token* last = first;
    U32 line = 0;
        ...
        if( context != 3 ){
            switch( c ){
                case '0' ... '9':
                    if( context == 0 ){
                        context = 1;
                        last = token_append(last, allocs(30), TokenType_NUMERIC, line);
                        last->value[0] = c;
                        continue;
                    }
                ...
                case ',':
                    ...
                    last = token_append(last, allocs(2), TokenType_SYMBOL, line);
                    last->value[0] = c;
                    break;
                case 'A' ... 'Z': case 'a' ... 'z': case '_':
                    ...
                    if( context == 0 ){
                        context = 2;
                        last = token_append(last, allocs(50), TokenType_WORD, line);
                        last->value[0] = c;
                        continue;
                    }
                    last->value[strlen(last->value)] = c;
                ...
                case LITERAL_CHAR:
                    if( i == 0 || code[i-1] != LITERAL_ESC_CHAR ){
                        if( context == 0 ){
                            context = 3;
                            last = token_append(last, allocs(100), TokenType_LITERAL, line);
                        }
                        else if( context == 3 ){
                            context = 0;
                            last->value = minmv(last->value);
